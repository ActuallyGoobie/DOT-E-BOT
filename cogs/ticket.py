...
class Ticket(commands.Cog):
    ...
    async def createticket(self, ctx, *args):
        ...
        with open('db/ticket.json', 'r') as file:
        ...
        if new_ticket in ticket_data:
            ...
            with open('db/ticket.json', 'w') as update_ticket_data:
                json.dump(ticket_data, update_ticket_data, indent=4)

        # Add new ticket
        else:
            ticket_data[new_ticket] = [channel_id]
            with open('db/ticket.json', 'w') as new_ticket_data:
    ...
    async def closeticket(self, ctx, mentioned_user):
        ...
        get_mentioned_role = [items.name for items in ctx.message.author.guild.roles if f'{items.id}' in f'{mentioned_role}'] 
